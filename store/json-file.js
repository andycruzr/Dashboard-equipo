...
module.exports = {
  ...
  async setState({ tareas, proyectos, hitos }) {
    return commit(s => { s.tareas = tareas; if (proyectos) s.proyectos = proyectos; s.hitos = hitos; return s; });
  },

  async reset() {
    const seed = JSON.parse(await fs.readFile(SEED, 'utf8'));
    return commit(s => { s.tareas = seed.tareas; s.proyectos = seed.proyectos; s.hitos = seed.hitos; return s; });
  },
