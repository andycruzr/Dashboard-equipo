...
app.put('/api/state', wrap(async (req, res) => {
  const { tareas, proyectos, hitos, personas, version } = req.body || {};
  ...
  const saved = await store.setState({ tareas, proyectos: proyectos || current.proyectos, hitos, personas: personas || current.personas });
  broadcast(saved);
