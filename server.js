...
/* Con AUTH=entra se valida el token corporativo en vez de la clave
   compartida. Si el servidor vive en Azure App Service con Easy Auth,
   deja AUTH sin definir: Azure valida antes de llegar aquí.          */
if (process.env.AUTH === 'entra') {
  app.use(require('./auth-entra').middleware());
} else if (CLAVE) {
  app.use((req, res, next) => {
    if (req.path === '/api/health') return next();          // para los health checks del hosting

    const cabecera = req.headers.authorization || '';
    const [tipo, b64] = cabecera.split(' ');
    if (tipo === 'Basic' && b64) {
      const [u, p] = Buffer.from(b64, 'base64').toString().split(':');
      // Comparación de longitud constante para no filtrar la clave por tiempos
...
(async function arrancar() {
  ...
  app.listen(PORT, () => {
    ...
    if (process.env.AUTH === 'entra') {
      ...
      console.warn('   Define CLAVE_ACCESO, o AUTH=entra para usar la cuenta corporativa.\n');
    }
  });
})();
